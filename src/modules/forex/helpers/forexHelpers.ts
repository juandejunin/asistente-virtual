import { ForexTrend, ForexDirection, CurrencyCode } from "../types";
import { currencyList } from "../../../config/currencyList"; // <-- reutilizamos la lista

// Validación runtime de CurrencyCode
export function isCurrencyCode(value: any): value is CurrencyCode {
  return typeof value === "string" && currencyList.includes(value as CurrencyCode);
}

// Builder de ForexTrend
export function buildForexTrend(
  ...
  const pairDirection: ForexDirection =
    pairDirectionOverride ?? (change > 0 ? "up" : change < 0 ? "down" : "flat");

  const currencyDirection: ForexDirection =
    pairDirection === "down" ? "up" : pairDirection === "up" ? "down" : "flat";
